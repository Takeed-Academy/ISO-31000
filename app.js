...
(function () {
  ...
  /* ------------------------------------------------------ session defaults
   * The setup screen was reduced to a single Start button, so these fix what
   * every session looks like. The engine still supports the other values —
   * change a constant here to bring an option back without touching the UI.
   *
   *   SESSION_SIZE       "all" for the whole bank, or a number such as 10 / 25
   *   SESSION_DIFFICULTY "all" | "basic" | "intermediate" | "advanced"
   *   SESSION_MODE       "study" reveals the answer per question via Check;
   *                      "exam"  hides everything until the session ends
   */
  var SESSION_SIZE = "all";
  ...
  function defaultState() {
    return { lang: DEFAULT_LANG, view: "setup", session: null };
  }

  function validMode(value) {
    return value === "study" || value === "exam" ? value : SESSION_MODE;
  }

  /* Resolves SESSION_SIZE against however many questions the filter leaves. */
  function sessionCount(available) {
    return SESSION_SIZE === "all"
      ? available
      : Math.min(Number(SESSION_SIZE) || available, available);
  }
  ...
  /* Forward navigation in study mode reveals an answered-but-unchecked
     question first, and only moves on once it has been seen. An unanswered
     question is skipped straight past, so nothing traps the learner. */
  function advance() {
    var question = currentQuestion();
    if (question && isStudyMode() &&
      Object.prototype.hasOwnProperty.call(state.session.answers, question.id) &&
      !state.session.revealed[question.id]) {
  ...
  function renderSetup() {
    ...
    var available = availableQuestions(SESSION_DIFFICULTY).length;
    ...
    host.innerHTML =
      ...
      '<p class="muted-text">' + esc(t("setupHint", { n: num(sessionCount(available)) })) + "</p>" +
      '<div class="button-row">' +
      '<button class="primary-button is-lead" type="button" data-action="start-session">' + icon("play") +
  ...
  function renderPractice() {
    ...
    /* Study mode folds "check" into Next: the first press on an answered
       question reveals it, the second press moves on. `willReveal` is what
       tells the Next button which of the two jobs it is doing right now. */
    ...
    host.innerHTML =
      ...
      /* On the last question Next stays live while it still has an answer to
         reveal, otherwise there would be no way to see it. */
      '<button class="' + (willReveal || revealed ? "primary-button" : "exam-nav-button") +
      '" type="button" data-action="next"' +
      (s.current === total - 1 && !willReveal ? " disabled" : "") +
      "><span>" + esc(t("next")) + "</span>" + icon("arrowNext") + "</button>" +
      "</div>" +
      '<div class="button-row">' +
  ...
  function handleAction(action) {
    switch (action) {
      case "start-session":
        startSession(SESSION_SIZE, SESSION_DIFFICULTY, SESSION_MODE);
        break;
      case "reveal":
  ...
  function onKeydown(event) {
    ...
    if (event.key === forward) { event.preventDefault(); advance(); }
      ...
      /* Enter mirrors Next: reveal, then move on. */
